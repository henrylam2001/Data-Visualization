...
```{r}
IPS_angle_catagory <- result1 %>%
  mutate(angle_catagory = case_when(
    angle_diff >= 150 & angle_diff <= 180 ~ "Very Cold", 
    angle_diff >= 120 & angle_diff < 150 ~ "Cold",
    angle_diff >= 90 & angle_diff < 120 ~ "Cool",
    angle_diff >= 60 & angle_diff < 90 ~ "Warm",
    angle_diff >= 30 & angle_diff < 60 ~ "Hot",
    angle_diff >= 0 & angle_diff < 30 ~ "Very Hot"
  )) %>% 
  select(
    1:5,           # select the first 4 columns to be the first
    angle_catagory,    # moves angle_diff to the 5th column spot      
    everything()   # Then adds the rest of the columns in order
  )
head(IPS_angle_catagory)
```
...
```
```{r}
# This graph sees if having the device measure while pointing to the router improve the signal strength
ggplot(IPS_angle_catagory, aes(x = dist, y = mean_signal)) +
  geom_point() +
  facet_grid(angle_catagory~mac) +
  geom_smooth()
```

```{r}

IPS_angle_prove <- IPS_angle_catagory %>% 
  group_by(angle_catagory, mac) %>% 
  summarise(average_signal = mean(mean_signal, na.rm = TRUE),
    median_signal = median(mean_signal, na.rm = TRUE),
    sd_signal = sd(mean_signal, na.rm = TRUE),
    min_signal = min(mean_signal, na.rm = TRUE),
    max_signal = max(mean_signal, na.rm = TRUE),
    IQR_signal = IQR(mean_signal, na.rm = TRUE),
    count = n(),
    .groups = "drop"  
  ) %>% 
  arrange(mac, desc(average_signal)) 

print(IPS_angle_prove, n = 36, quote = FALSE)
...
IPS_angle_prove$angle_catagory <- fct_relevel(
  #IPS_angle_prove$angle_catagory,
  #"Very Cold","Cold","Cool","Warm","Hot","Very Hot")  # Desired order

ggplot(data = IPS_angle_prove, aes(x = angle_catagory, y = average_signal)) +
