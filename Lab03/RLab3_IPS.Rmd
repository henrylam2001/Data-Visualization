...
IPS_data_change_class <- IPS_sampledata %>%
...
IPS_data_change_time<-IPS_data_change_class %>% mutate(time = posixct_func(time))

IPS_data_change_time
```
...
IPS_join <- IPS_data_change_time %>% inner_join(mac_locs, by = c("mac"))
IPS_join
```
...
unique(IPS_join$orientation)

# Recodes the data in orientation to conform to specific values, aka 0,45,90,135,180,225,270,315
IPS_ori <- IPS_join %>%
