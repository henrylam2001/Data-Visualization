...
# Checks to make sure there are no angle differences above or below zero, or NA values 
...
IPS_angle_catagory <- result1 %>%
  mutate(angle_catagory = case_when(
    angle_diff >= 150 & angle_diff <= 180 ~ "Very weak", 
    angle_diff >= 120 & angle_diff < 150 ~ "Weak",
    angle_diff >= 90 & angle_diff < 120 ~ "Poor",
    angle_diff >= 60 & angle_diff < 90 ~ "Mild",
    angle_diff >= 30 & angle_diff < 60 ~ "Strong",
    angle_diff >= 0 & angle_diff < 30 ~ "Very Strong"
  )) %>% 
  select(
...
filtered_data <- IPS_angle %>%
  slice(c(5,18,40)) %>% select(posX,posY,rec_orient,angle,x,y,dist,mean_signal)
print(filtered_data)
```
