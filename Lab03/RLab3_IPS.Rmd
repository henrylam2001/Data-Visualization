...
IPS_sampledata <-IPS_sampledata %>% mutate(time = posixct_func(time))
...
```{r filter_by_macs}
#create a new variable to filter the six access points
...
#apply the filter to keep only the six access points as required
IPS_sampledata <- IPS_sampledata %>% filter(mac %in% MAC_addresses)
IPS_sampledata
```
...
```{r bin_orientation}
# your code here
# Identifies all unique orientation observations (to get a sense of what the values are)
unique(IPS_sampledata$orientation)

# Recodes the data in orientation to conform to specific values: 0,45,90,135,180,225,270,315
IPS_sampledata <- IPS_sampledata %>%
  mutate(rec_orient = case_when(
    orientation <= 45/2 ~ 0,
    orientation <= 135/2 ~ 45,
    orientation <= 225/2 ~ 90,
    orientation <= 315/2 ~ 135,
    orientation <= 405/2 ~ 180,
    orientation <= 495/2 ~ 225,
    orientation <= 585/2 ~ 270,
    orientation <= 675/2 ~ 315,
    orientation == 360 ~ 0
  )) 
IPS_sampledata
```
...
IPS_sampledata <- IPS_sampledata %>% inner_join(mac_locs, by = c("mac"))
IPS_sampledata
```
